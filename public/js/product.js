...
productImages.forEach((item,i) =>{
    item.addEventListener('click', () => {
        ...
        item.classList.add('active');//adding active class to current or clicked
        productImageSlide.style.backgroundImage = `url('${item.src}')`;
        activeImageSlide = i; //updating image slider variable
...
sizeBtns.forEach((item,i) => {
    item.addEventListener('click', () => {
        ...
        item.classList.add('check');//adding checked class to click button
        checkedBtn = i;//updating variable
        size = item.innerHTML;
    })
})
